...
import SearchSorumluPers from './SearchTable/SearchSorumluPers'
import { Alert, Stack } from 'react-bootstrap';
import Pagination from '@mui/material/Pagination';
import { TextField } from '@mui/material';

const SorumluPers = (props) => {
...
const totalPagesNum = Math.ceil(talepList.filter(tal => {
 ...
 function search(){
    ...
    return (
       ...
       <h5 className="card-title">Onay Bekleyen Personeller</h5>
        ...
        return tal.personelID !== props.PersonelAd && tal.talepDurum === 0 && tal.talepTahminiDeger <= props.YetkiliButce
        &&  perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
...
<TextField id="filled-basic" label="Arama..." variant="filled" onChange={(e) => setQ(e.target.value.toLowerCase())} />
            ...
            <table className="table mt-1 table-bordered">
                ...
                {/* {
                    ...
                    <SearchSorumluPers startDate={startDate} endDate={endDate} talepList={search(talepList)} indexOfLastEmployee={indexOfLastEmployee} PersonelID={props.PersonelID} indexOfFirstEmployee={indexOfFirstEmployee} PersonelAd={props.PersonelAd} />
