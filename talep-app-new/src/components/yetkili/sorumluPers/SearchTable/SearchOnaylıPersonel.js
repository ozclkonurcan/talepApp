...
const SearchOnaylıPersonel = ({startDate,endDate, tal,PersonelID, talepList,PersonelAd,currentEmployee,indexOfFirstEmployee,indexOfLastEmployee }) => {
    ...
    <>
 
     {
                    ...
                    talepList.sort((a,b) => (a.talepTarih > b.talepTarih ? -1 : 1)).filter(tal => {
                        ...
                        yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
                        || tal.talepTarih === startDate && tal.personelID !== PersonelAd && tal.talepDurum === 1 &&
                        yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
...
export default SearchOnaylıPersonel;