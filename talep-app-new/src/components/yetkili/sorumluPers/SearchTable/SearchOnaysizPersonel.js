...
const SearchOnaysizPersonel = ({startDate,endDate, tal,PersonelID,talepList,PersonelAd,currentEmployee,indexOfFirstEmployee,indexOfLastEmployee }) => {
    ...
    <>
   
     {
                    ...
                    talepList.sort((a,b) => (a.talepTarih > b.talepTarih ? -1 : 1)).filter(tal => {
                        ...
                        tal.personelID !== PersonelAd && tal.talepDurum === 2 &&
                        yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID): 
                         PersonelAd && tal.talepTarih >= startDate && tal.talepTarih <= endDate && tal.personelID !== PersonelAd && tal.talepDurum === 2 &&
                        yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
                        || tal.talepTarih === startDate && tal.personelID !== PersonelAd && tal.talepDurum === 2 &&
                        yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
                    ...
                    <tr key={tal.talepID}>
                        ...
                        <td>{tal.talepTahminiDeger} <span className="badge bg-success rounded-pill float-end"> {tal.paraBirimi}</span></td>
...
export default SearchOnaysizPersonel;