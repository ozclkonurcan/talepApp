...
import SearchOnaylıPersonel from './SearchTable/SearchOnaylıPersonel'
import { Alert, Stack } from 'react-bootstrap';
import Pagination from '@mui/material/Pagination';
import { TextField } from '@mui/material';
const OnaylıPersonelDurum = (props) => {
...
const totalPagesNum = Math.ceil(talepList.filter(tal => {
   return tal.personelID !== props.PersonelAd && tal.talepDurum===1
...
<div class="card p-1">
   ...
   <h5 className="card-title">Onaylanmış Personeller</h5>
<div className="container">
<Alert style={{height:"15px",textAlign:"center",display:"flex",justifyContent:"center",alignItems:"center"}} className="alert alert-success">Toplam Talep Tutarı : {talepList.filter(tal => {
        return tal.personelID !== props.PersonelAd && tal.talepDurum === 1
        &&  perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
...
<table className="table mt-1 table-bordered">
  ...
  {/* {
      ...
      talepList.filter(talp => {
          return talp.personelID !== props.PersonelAd && talp.talepDurum===1
      }).map(tal => 
      <tr key={tal.talepID}>
      ...
      <SearchOnaylıPersonel startDate={startDate} endDate={endDate} talepList={search(talepList)} PersonelID={props.PersonelID} indexOfLastEmployee={indexOfLastEmployee} indexOfFirstEmployee={indexOfFirstEmployee} PersonelAd={props.PersonelAd} />
...
export default OnaylıPersonelDurum
