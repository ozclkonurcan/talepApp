...
import SearchOnaylı from '../SearchTable/SearchOnaylı';
//import Pagination from '../Pagination/Pagination';

import Pagination from '@mui/material/Pagination';
import { Stack } from 'react-bootstrap';
import { TextField } from '@mui/material';
const OnaylıPersonel = (props) => {
...
const totalPagesNum = Math.ceil(talepList.filter(tal => {
 ...
 function search(){
    ...
    return(
        ...
        row.talepTarih.toString().toLowerCase().indexOf(q,'') > - 1
        ))
    )
}
...
<h5 class="card-title">Personel Onaylanmış Talep Formları</h5>
...
<table className="table mt-1 table-bordered">
  ...
  {/* {
      ...
      <SearchOnaylı startDate={startDate} endDate={endDate} talepList={search(talepList)} indexOfLastEmployee={indexOfLastEmployee} indexOfFirstEmployee={indexOfFirstEmployee} PersonelAd={props.PersonelAd} />
