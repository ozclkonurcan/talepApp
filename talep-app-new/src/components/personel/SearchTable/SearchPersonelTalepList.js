...
const SearchPersonelTalepList = ({ startDate,endDate, tal, talepList,PersonelAd,currentEmployee,indexOfFirstEmployee,indexOfLastEmployee }) => {
    ...
    <>
     {
                    ...
                    talepList.sort((a,b) => (a.talepTarih > b.talepTarih ? -1 : 1)).filter(tal => {
                        return startDate === '' && endDate === '' ? tal.personelID === PersonelAd  : PersonelAd && tal.talepTarih >= startDate && tal.talepTarih <= endDate && tal.personelID === PersonelAd 
                        || tal.talepTarih === startDate && tal.personelID === PersonelAd
                    }).slice(indexOfFirstEmployee,indexOfLastEmployee).map(tal => 
                
                    <tr key={tal.talepID}>
                        <td>{tal.talepAd}</td>
                        <td>{tal.talepAciklamasi}</td>
                        <td>{tal.talepMiktar} / {tal.agirlikBirimi}</td>
...
export default SearchPersonelTalepList;