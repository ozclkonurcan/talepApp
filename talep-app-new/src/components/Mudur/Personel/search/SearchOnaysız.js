    ...
    <>
    
     {
                    ...
                    talepList.sort((a,b) => (a.talepAd < b.talepAd ? -1 : 1)).filter(tal => {
                        ...
                        tal.personelID != PersonelAd && tal.talepDurum == 2 && tal.personelStatuID == 1
                        &&  perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID) : 
                         PersonelAd && tal.talepTarih >= startDate && tal.talepTarih <= endDate &&   tal.personelID != PersonelAd && tal.talepDurum == 2 && tal.personelStatuID == 1
                        &&  perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
                        || tal.talepTarih === startDate &&   tal.personelID != PersonelAd && tal.talepDurum == 2 && tal.personelStatuID == 1
                        &&  perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
