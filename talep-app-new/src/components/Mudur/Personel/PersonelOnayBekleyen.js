...
import SearchYoneticiOnay from '../Personel/search/SearchOnayBekleyen';
//import Pagination from '../../personel/Pagination/Pagination';
import Pagination from '@mui/material/Pagination';
import { TextField } from '@mui/material';

const PersonelOnayBekleyen = (props) => {
...
const totalPagesNum = Math.ceil(talepList.filter(tal => {
 ...
 function search(){
    ...
    return(
        ...
        row.departmanID.toUpperCase().indexOf(q) > - 1 ||
        row.personelID.toLowerCase().indexOf(q) > - 1 ||
        row.personelID.toUpperCase().indexOf(q) > - 1 ||
    ...
    return (
        ...
        <div class="card p-1">
            ...
            <h5 className="card-title">Personel Onay Bekleyen</h5>
        ...
        return tal.personelID !== props.PersonelAd && tal.talepDurum === 0 && tal.personelStatuID===1
        && tal.talepTahminiDeger >= 10000
        && perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
...
export default PersonelOnayBekleyen
