    ...
    <>
    
     {
                    ...
                    talepList.sort((a,b) => (a.talepTarih > b.talepTarih ? -1 : 1)).filter(tal => {
                        ...
                        tal.personelID != PersonelAd && tal.personelStatuID ===2  && tal.talepDurum === 0
                        &&   perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID) : 
                         PersonelAd && tal.talepTarih >= startDate && tal.talepTarih <= endDate &&       tal.personelID != PersonelAd && tal.personelStatuID ===2  && tal.talepDurum === 0
                        &&   perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
                        || tal.talepTarih === startDate &&       tal.personelID != PersonelAd && tal.personelStatuID ===2  && tal.talepDurum === 0
                        &&   perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
