...
const SearchModalPers = ({startDate,endDate, tal,PersonelID,PersonelResim, talepList,PersonelAd,currentEmployee,indexOfFirstEmployee,indexOfLastEmployee }) => {
    ...
    <>
     {
                    ...
                    talepList.sort((a,b) => (a.talepAd < b.talepAd ? -1 : 1)).filter(tal => {
                        ...
                        tal.personelID !== PersonelAd && tal.talepDurum === 0 && tal.personelStatuID === 1 &&
                         perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)&&
                         tal.talepTahminiDeger > 10000 && durumList.some(drm =>drm.personelAd === tal.personelID && drm.durumID === "Aktif") : 
                         PersonelAd && tal.talepTarih >= startDate && tal.talepTarih <= endDate && tal.personelID !== PersonelAd && tal.talepDurum === 0 && tal.personelStatuID === 1 &&
                         perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)&&
                         tal.talepTahminiDeger > 10000 && durumList.some(drm =>drm.personelAd === tal.personelID && drm.durumID === "Aktif")
                        || tal.talepTarih === startDate && tal.personelID !== PersonelAd && tal.talepDurum === 0 && tal.personelStatuID === 1 &&
                         perList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)&&
                         tal.talepTahminiDeger > 10000 && durumList.some(drm =>drm.personelAd === tal.personelID && drm.durumID === "Aktif")
...
export default SearchModalPers;