...
const SearchYoneticiOnay = ({startDate,endDate, tal,PersonelID,PersonelResim, talepList,PersonelAd,currentEmployee,indexOfFirstEmployee,indexOfLastEmployee }) => {
    ...
    <>
     {
                    ...
                    talepList.sort((a,b) => (a.talepAd < b.talepAd ? -1 : 1)).filter(tal => {
                        ...
                        tal.personelID != PersonelAd   
                        && yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)  : 
                        PersonelAd && tal.talepTarih >= startDate && tal.talepTarih <= endDate && tal.personelID != PersonelAd   
                        && yetList.some(per => per.sektorID === tal.sektorID && per.sirketID === tal.sirketID && per.departmanID === tal.departmanID)
